...
1. 오늘의 총 지출 금액 표현하기
2. 날짜 검색하기 -> 검색한 날짜 지출 목록에 표현, 총 지출 금액도 표현
3. 월(Month) 검색 -> 리스트 없이 그 달의 지출 금액만 표현
4. 입금 추가
  1) (1,2번의)오늘의 총 지출은 입금이 있을 경우 총 입금도 표현
  2) (3번의)그 달의 지출은 입금 지출 모두 표현하고 총 사용 금액(입금-지출 금액)도 표현
