...
// ‹ 2026년 9월 ⌄ › — 화살표로 한 달씩, 가운데를 누르면 월 선택 시트. 홈·내역·통계가 같은 달을 공유
export function MonthNavigator({ size = 'md', showThisMonth, onChange }: MonthNavigatorProps) {
  ...
  return (
    <View style={styles.row}>
      ...
      <Pressable onPress={() => setPickerVisible(true)} style={styles.label} hitSlop={6} accessibilityRole="button" accessibilityLabel={`${year}년 ${monthIndex + 1}월, 월 선택`}>
        <Text style={[size === 'lg' ? typography.heading : typography.bodyBold, { color: colors.text }]}>
          {year}년 {monthIndex + 1}월
        </Text>
        <Feather name="chevron-down" size={size === 'lg' ? 16 : 14} color={colors.textSecondary} />
      </Pressable>
...
const makeStyles = ({ colors, radius, typography }: Theme) =>
  StyleSheet.create({
    ...
    label: { flexDirection: 'row', alignItems: 'center', gap: 3 },
